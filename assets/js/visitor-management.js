(function () {
  ...
  const state = {
    ...
    total: 0,
    totalPages: 1,
    filters: { search: '', visitor_type: 'all', visit_status: 'all', department_id: 'all', host_employee_id: 'all' },
  ...
  const columns = {
    visitorType: { label: 'Type', filterKey: 'visitor_type', sortKey: 'visitor_type', options: () => (state.options.visitor_types || []).map(x => ({ value: x, label: title(x) })) },
    destination: { label: 'Destination', filterKey: 'department_id', options: () => (state.options.departments || []).map(x => ({ value: String(x.id), label: x.name || x.code })) },
    host: { label: 'Host', filterKey: 'host_employee_id', options: () => (state.options.host_employees || []).map(x => ({ value: String(x.id), label: x.full_name || x.employee_number })) },
    status: { label: 'Status', filterKey: 'visit_status', sortKey: 'visit_status', options: () => (state.options.visit_statuses || []).map(x => ({ value: x, label: title(x) })) }
  };
  ...
  function resetVisible() { qs('#visitor-reset-filters')?.classList.toggle('hidden', !activeFilters()); }
  function renderSummary(summary = {}) { qs('#visitor-summary').innerHTML = [["Today's Visitors", summary.today], ['Pending Verification', summary.pending_review], ['Checked In', summary.checked_in], ['Checked Out', summary.checked_out], ['Applicants Today', summary.applicants_today]].map(([k, v]) => `<span><strong>${Number(v || 0)}</strong>${esc(k)}</span>`).join(''); }
  function renderHeaders() {
    Object.entries(columns).forEach(([key, cfg]) => {
      const h = qs('[data-column="' + key + '"]');
      if (!h) return;
      const filterable = Boolean(cfg.filterKey && cfg.options);
      const fv = filterable ? state.filters[cfg.filterKey] : 'all';
      const sorted = Boolean(cfg.sortKey && state.sort === cfg.sortKey);
      const filtered = filterable && fv !== 'all';
      h.innerHTML = `<div class="facility-column-menu"><button class="facility-column-trigger ${(sorted || filtered) ? 'active' : ''}" type="button" aria-haspopup="menu" aria-expanded="${state.openColumnMenu === key}" data-column-menu-toggle="${key}"><span>${esc(cfg.label)}</span>${filtered ? '<span class="facility-filter-dot" aria-label="Filtered"></span>' : ''}<span class="facility-sort-indicator ${sorted ? 'facility-sort-' + state.direction : ''}" aria-hidden="true"></span></button><div class="facility-column-dropdown ${state.openColumnMenu === key ? '' : 'hidden'}" role="menu">${headerMenu(key, cfg)}</div></div>`;
    });
    positionColumn();
  }
  function headerMenu(key, cfg) {
    const filterable = Boolean(cfg.filterKey && cfg.options);
    const fv = filterable ? state.filters[cfg.filterKey] : 'all';
    const sortGroup = cfg.sortKey ? `<div class="facility-column-menu-group"><span>Sort</span><button type="button" role="menuitem" data-column-sort="${key}" data-sort-direction="asc">Sort Ascending</button><button type="button" role="menuitem" data-column-sort="${key}" data-sort-direction="desc">Sort Descending</button>${state.sort === cfg.sortKey ? `<button type="button" role="menuitem" data-column-clear-sort="${key}">Clear Sort</button>` : ''}</div>` : '';
    const filterGroup = filterable ? `<div class="facility-column-menu-group"><span>Filter</span><button type="button" role="menuitem" data-column-filter="${key}" data-filter-value="all">All</button>${cfg.options().map(o => `<button class="${String(fv) === String(o.value) ? 'selected' : ''}" type="button" role="menuitem" data-column-filter="${key}" data-filter-value="${esc(o.value)}">${esc(o.label)}</button>`).join('')}${fv !== 'all' ? `<button type="button" role="menuitem" data-column-clear-filter="${key}">Clear Filter</button>` : ''}</div>` : '';
    return sortGroup + filterGroup;
  }
  ...
  function renderTable() {
    ...
    if (!state.rows.length) {
      ...
      empty.innerHTML = `<span class="material-symbols-outlined" aria-hidden="true">badge</span><strong>${activeFilters() ? 'No visitors match the current search or filters.' : 'No visitor submissions are waiting in the queue.'}</strong><span>${can('visitors.create_walkin') ? '<button class="facility-text-button" type="button" data-open-walkin>Manual Registration</button>' : 'Visitor submissions will appear here after public registration.'}</span>`;
      return;
    }
  ...
  function openWalkin() { const d = moveToTopLayer(qs('#visitor-dialog')); d.hidden = false; d.innerHTML = `<form class="facility-dialog-panel visitor-form" data-walkin-form><div class="facility-details-modal-header"><div><p>Contingency workflow only</p><h2>Manual Registration</h2></div><button class="facility-details-modal-close" type="button" data-close-dialog aria-label="Close form">&times;</button></div><div class="visitor-form-body"><div class="facility-form-error" data-form-error hidden></div><div class="facility-form-grid">${field('full_name', 'Full Name')}${field('email_address', 'Email', 'email')}${field('mobile_number', 'Mobile')}${field('organization_name', 'Organization / School')}${selectField('visitor_type', 'Visitor Type', state.options.visitor_types, 'Select type')}${field('visit_purpose', 'Purpose')}${selectField('destination_department_reference_id', 'Destination Department', state.options.departments, 'Optional')}${selectField('host_employee_reference_id', 'Host Employee', state.options.host_employees, 'Optional')}${selectField('facility_space_id', 'Facility Space', state.options.facility_spaces, 'Optional')}${field('scheduled_start_at', 'Scheduled Start', 'datetime-local')}${field('scheduled_end_at', 'Scheduled End', 'datetime-local')}${field('applicant_reference', 'Applicant Reference')}${selectField('identification_type', 'Identification Type', state.options.identity_document_types, 'Optional')}${field('identification_last4', 'ID Last Four')}</div><label class="facility-field"><span>Description</span><textarea name="visit_description" rows="3"></textarea></label><label class="facility-field"><span>Remarks</span><textarea name="remarks" rows="3"></textarea></label></div><div class="facility-dialog-actions"><button class="btn-secondary dashboard-action-button" type="button" data-close-dialog>Cancel</button><button class="btn-primary dashboard-action-button" type="submit">Manual Registration</button></div></form>`; d.querySelector('input,select,textarea,button')?.focus(); }
  async function submitWalkin(form) { const btn = form.querySelector('[type="submit"]'), box = form.querySelector('[data-form-error]'); btn.disabled = true; box.hidden = true; qsa('.facility-field-error').forEach(x => x.remove()); const data = Object.fromEntries(new FormData(form).entries()); try { const r = await window.FAMApi.request('../api/visitors/create-walkin.php', { method:'POST', body:data }); closeDialog(); toast('Manual visitor registration submitted.'); await load(); openDetails(r.data?.item?.id); } catch (e) { if (e.status === 422) { Object.entries(e.payload?.data?.errors || e.errors || {}).forEach(([k, m]) => { const f = form.querySelector(`[name="${k}"]`); f?.insertAdjacentHTML('afterend', `<span class="facility-field-error">${esc(m)}</span>`); }); } box.textContent = e.message || 'Unable to register visitor.'; box.hidden = false; } finally { btn.disabled = false; } }
  ...
  function detail(k, v) { return `<dl class="facility-detail-row"><dt>${esc(k)}</dt><dd>${esc(v || 'Not applicable')}</dd></dl>`; }
  ...
  function detailsHtml(item) {
    ...
    const timeline = `${detail('Scheduled Start', scheduleValue(item.scheduled_start_at))}${detail('Scheduled End', scheduleValue(item.scheduled_end_at))}${detail('Time In', fmt(item.actual_check_in_at))}${detail('Time Out', fmt(item.actual_check_out_at))}${detail('Duration', duration(item))}`;
    ...
    const badgeSection = hasBadge ? `<details class="visitor-detail-disclosure"><summary>Badge Information</summary>${detail('Badge', badgeInfo)}${detail('Badge Status', item.badge?.status)}</details>` : '';
    ...
    return `<div class="facility-details-modal-panel visitor-details-panel"><div class="facility-details-modal-header visitor-details-header"><div><p>Visitor Reference Number</p><span class="facility-details-modal-request-number">${esc(item.visitor_reference_number)}</span><h2>${esc(clean(v.full_name))}</h2></div><button class="facility-details-modal-close" type="button" data-close-drawer aria-label="Close visitor details">&times;</button></div><div class="facility-details-modal-body visitor-details-body"><div class="visitor-detail-accordion"><details class="visitor-detail-disclosure" open><summary>Visitor Information</summary>${detail('Name', clean(v.full_name))}${detail('Type', title(v.visitor_type))}${detail('Email', v.email_address)}${detail('Mobile', v.mobile_number)}${detail('Organization / School', v.organization_name)}</details><details class="visitor-detail-disclosure" open><summary>Visit Details</summary>${detail('Purpose', item.visit_purpose)}${detail('Status', statusLabel(item.visit_status))}${detail('Approval', title(item.approval_status))}${detail('Department', item.destination_department?.name)}${detail('Host', item.host?.full_name)}${detail('Facility Space', item.facility_space?.name)}</details><details class="visitor-detail-disclosure" open><summary>Schedule and Attendance</summary>${timeline}</details>${badgeSection}${remarksSection}<details class="visitor-detail-disclosure"><summary>Identity Verification</summary>${detail('Verified', idv.verified ? 'Yes' : 'No')}${detail('ID Type', idv.identification_type)}${detail('ID Last Four', idv.identification_last4)}${detail('Verified At', fmt(idv.verified_at))}</details><details class="visitor-detail-disclosure"><summary>Registration Details</summary>${detail('Registration Source', title(item.registration_source))}${detail('Applicant Reference', item.applicant_reference)}${detail('Company / School', item.company_or_school)}</details><details class="visitor-detail-disclosure"><summary>Activity History</summary>${historyHtml}</details></div></div></div>`;
  }
  ...
  function resetFilters() { state.filters = { search:'', visitor_type:'all', visit_status:'all', department_id:'all', host_employee_id:'all' }; state.sort = 'scheduled_start_at'; state.direction = 'desc'; state.page = 1; state.openColumnMenu = null; const s = qs('#visitor-search'); if (s) s.value = ''; load(); }
  function bind() {
    ...
    qs('#visitor-register-walkin')?.addEventListener('click', openWalkin);
    ...
    qs('#visitor-search')?.addEventListener('input', e => { state.filters.search = e.target.value; state.page = 1; clearTimeout(state.timer); state.timer = setTimeout(load, 300); resetVisible(); });
    qs('#visitor-reset-filters')?.addEventListener('click', resetFilters);
    ...
    document.addEventListener('click', e => {
      if (e.target.closest('[data-open-walkin]')) return openWalkin();
      const columnToggle = e.target.closest('[data-column-menu-toggle]');
      if (columnToggle) { state.openColumnMenu = state.openColumnMenu === columnToggle.dataset.columnMenuToggle ? null : columnToggle.dataset.columnMenuToggle; qsa('[data-visitor-menu]').forEach(x => x.classList.add('hidden')); return renderTable(); }
      const columnSort = e.target.closest('[data-column-sort]');
      if (columnSort) { const cfg = columns[columnSort.dataset.columnSort]; state.sort = cfg.sortKey; state.direction = columnSort.dataset.sortDirection; state.page = 1; state.openColumnMenu = null; return load(); }
      const clearSort = e.target.closest('[data-column-clear-sort]');
      if (clearSort) { state.sort = 'scheduled_start_at'; state.direction = 'desc'; state.page = 1; state.openColumnMenu = null; return load(); }
      const columnFilter = e.target.closest('[data-column-filter]');
      if (columnFilter) { const cfg = columns[columnFilter.dataset.columnFilter]; state.filters[cfg.filterKey] = columnFilter.dataset.filterValue; state.page = 1; state.openColumnMenu = null; return load(); }
      const clearFilter = e.target.closest('[data-column-clear-filter]');
      if (clearFilter) { const cfg = columns[clearFilter.dataset.columnClearFilter]; state.filters[cfg.filterKey] = 'all'; state.page = 1; state.openColumnMenu = null; return load(); }
      ...
      if (menu) { state.openColumnMenu = null; renderHeaders(); return window.FAMTableMenus?.toggle(menu, qs(`[data-visitor-menu="${CSS.escape(menu.dataset.openVisitorMenu)}"]`)); }
      ...
      if (!e.target.closest('.facility-action-menu') && !e.target.closest('.facility-action-dropdown') && !e.target.closest('.facility-column-menu')) { state.openColumnMenu = null; renderHeaders(); window.FAMTableMenus?.close(); }
    });
    document.addEventListener('submit', e => { if (e.target.matches('[data-walkin-form]')) { e.preventDefault(); submitWalkin(e.target); } if (e.target.matches('[data-checkin-form]')) { e.preventDefault(); submitCheckin(e.target).catch(err => toast(err.message || 'Check-in failed.')); } });
    document.addEventListener('keydown', e => { if (e.key === 'Escape') { if (!qs('#visitor-dialog')?.hidden) closeDialog(); else if (!qs('#visitor-details-drawer')?.hidden) closeDrawer(); else if (state.openColumnMenu) { state.openColumnMenu = null; renderHeaders(); } } });
  }
