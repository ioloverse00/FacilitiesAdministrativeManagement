(function () {
  ...
  const state = {
    ...
    pagination: { page: 1, per_page: pageSize, total: 0, total_pages: 1 }, loading: true, tableLoading: false, loaded: false, error: false, accessDenied: false, openActionMenu: null, openColumnMenu: null, detailsId: null, searchTimer: null
  };
  ...
  function renderTable() {
    ...
    const body = qs('#facility-requests-table'), count = qs('#facility-table-count'), pager = qs('.facility-pagination');
    renderHeaders();
    ...
    pager.classList.remove('hidden'); qs('#facility-page-status').textContent = `Page ${state.pagination.page || state.page} of ${pages}`; qs('#facility-prev-page').disabled = state.page <= 1; qs('#facility-next-page').disabled = state.page >= pages;
  ...
  function accessDenied() { qs('#facility-requests-table').innerHTML = tableStateRow('You do not have permission to view facility requests.', 'lock'); }
  function actions(row) { const a = []; if (can('facility_requests.view')) a.push(['view','View Details']); if (can('facility_requests.edit') && ['DRAFT','SUBMITTED','PENDING_APPROVAL','APPROVED'].includes(row.status)) a.push(['edit','Edit']); if (can('facility_requests.approve') && row.status === 'PENDING_APPROVAL') { a.push(['approve','Approve']); a.push(['reject','Reject']); } if (can('facility_requests.assign') && ['SUBMITTED','APPROVED','ASSIGNED'].includes(row.status)) a.push(['assign', row.assigned_to ? 'Update Assignment' : 'Assign']); if ((can('facility_requests.edit') || can('facility_requests.assign')) && row.status === 'ASSIGNED') a.push(['start','Start Work']); if (can('facility_requests.complete') && row.status === 'IN_PROGRESS') a.push(['complete','Complete']); if (can('facility_requests.verify') && row.status === 'COMPLETED') a.push(['verify','Verify']); if ((can('facility_requests.verify') || can('facility_requests.manage')) && row.status === 'VERIFIED') a.push(['close','Close']); if (can('facility_requests.manage') && !['COMPLETED','VERIFIED','CLOSED','CANCELLED','REJECTED'].includes(row.status)) a.push(['cancel','Cancel Request']); return a.map(x => ({ key: x[0], label: x[1] })); }
  ...
  function openCreate() { ensureDialog(); const d = qs('#facility-dialog'); d.hidden = false; d.innerHTML = formHtml('New Facility Request', 'Create Request', {}); bindForm(null); d.querySelector('input, select, textarea, button')?.focus(); }
