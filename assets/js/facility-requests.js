(function () {
  ...
  const columns = {
    ...
    slaState: { label: 'SLA', filterKey: 'sla_status', sortKey: 'sla', options: () => [{ value: 'ON_TRACK', label: 'On Track' }, { value: 'DUE_SOON', label: 'Due Soon' }, { value: 'OVERDUE', label: 'Overdue' }, { value: 'NOT_APPLICABLE', label: 'Not applicable' }] },
    assignedTo: { label: 'Assigned To', filterKey: 'assigned_to', options: () => (state.options.assignees || []).map(x => ({ value: String(x.id), label: x.full_name })) },
  ...
  async function auth() { try { const r = await window.FAMApi.me(); state.user = r.user; state.accessDenied = !can('facility_requests.view'); } catch (e) { if (e.status === 401) window.location.href = `authentication_card_component_standard.html?next=${encodeURIComponent(window.location.pathname + window.location.search)}`; else throw e; } }
  ...
  function params() { const p = new URLSearchParams({ page: String(state.page), per_page: String(pageSize), sort: state.sortKey, direction: state.sortDirection }); if (state.search.trim()) p.set('search', state.search.trim()); ['status','priority','category_id','department_id','assigned_to','sla_status'].forEach(k => { if (state[k] !== 'all') p.set(k, state[k]); }); const d = dateParams(); if (d.date_from) p.set('date_from', d.date_from); if (d.date_to) p.set('date_to', d.date_to); return p; }
  ...
  function renderTable() {
    ...
    body.innerHTML = state.rows.map(row => `<tr data-request-id="${row.id}"><td class="facility-request-number"><button type="button" class="facility-link-button" data-open-details="${row.id}"><strong>${esc(row.request_number || 'Not assigned')}</strong></button></td><td><div class="facility-subject-cell"><button type="button" class="facility-link-button" data-open-details="${row.id}"><strong>${esc(row.subject || 'Untitled request')}</strong></button></div></td><td>${esc(row.category?.name || 'No category')}</td><td>${badge(row.priority || 'NORMAL', 'priority')}</td><td>${badge(row.status || 'SUBMITTED', 'status')}</td><td>${slaBadge(row)}</td><td>${esc(row.assigned_to?.full_name || 'Unassigned')}</td><td class="facility-date-cell">${esc(fmt(row.created_at))}</td><td class="facility-actions-cell"><div class="facility-action-menu"><button class="facility-action-toggle" type="button" aria-haspopup="menu" aria-expanded="${state.openActionMenu === String(row.id)}" aria-label="Actions for ${esc(row.request_number)}" data-action-menu-toggle="${row.id}">&#8942;</button><div class="facility-action-dropdown ${state.openActionMenu === String(row.id) ? '' : 'hidden'}" role="menu">${actions(row).map(a => `<button type="button" role="menuitem" data-request-action="${a.key}" data-request-id="${row.id}">${esc(a.label)}</button>`).join('')}</div></div></td></tr>`).join('');
    positionAction();
  }
  ...
  function actions(row) { const a = []; if (can('facility_requests.view')) a.push(['view','View Details']); if (can('facility_requests.edit') && ['DRAFT','SUBMITTED','PENDING_APPROVAL','APPROVED'].includes(row.status)) a.push(['edit','Edit']); if (can('facility_requests.assign') && ['SUBMITTED','APPROVED','ASSIGNED'].includes(row.status)) a.push(['assign', row.assigned_to ? 'Update Assignment' : 'Assign']); if ((can('facility_requests.edit') || can('facility_requests.assign')) && row.status === 'ASSIGNED') a.push(['start','Start Work']); if (can('facility_requests.complete') && row.status === 'IN_PROGRESS') a.push(['complete','Complete']); if (can('facility_requests.verify') && row.status === 'COMPLETED') a.push(['verify','Verify']); if ((can('facility_requests.verify') || can('facility_requests.manage')) && row.status === 'VERIFIED') a.push(['close','Close']); if (can('facility_requests.manage') && !['COMPLETED','VERIFIED','CLOSED','CANCELLED'].includes(row.status)) a.push(['cancel','Cancel Request']); a.push(['history','View Timeline']); return a.map(x => ({ key: x[0], label: x[1] })); }
  function renderHeaders() { Object.entries(columns).forEach(([key, cfg]) => { const h = qs(`[data-column="${key}"]`); if (!h) return; const fv = state[cfg.filterKey], sk = sortMap[cfg.sortKey || key] || cfg.sortKey || key, sorted = state.sortKey === sk, filtered = fv !== 'all'; h.innerHTML = `<div class="facility-column-menu"><button class="facility-column-trigger ${sorted || filtered ? 'active' : ''}" type="button" aria-haspopup="menu" aria-expanded="${state.openColumnMenu === key}" data-column-menu-toggle="${key}"><span>${esc(cfg.label)}</span>${filtered ? '<span class="facility-filter-dot" aria-label="Filtered"></span>' : ''}<span class="facility-sort-indicator ${sorted ? `facility-sort-${state.sortDirection}` : ''}" aria-hidden="true"></span></button><div class="facility-column-dropdown ${state.openColumnMenu === key ? '' : 'hidden'}" role="menu">${headerMenu(key, cfg)}</div></div>`; }); positionColumn(); }
  function headerMenu(key, cfg) { const fv = state[cfg.filterKey], sk = sortMap[cfg.sortKey || key] || cfg.sortKey || key; const opts = cfg.options(); return `<div class="facility-column-menu-group"><span>Sort</span><button type="button" role="menuitem" data-column-sort="${key}" data-sort-direction="asc">Sort Ascending</button><button type="button" role="menuitem" data-column-sort="${key}" data-sort-direction="desc">Sort Descending</button>${state.sortKey === sk ? `<button type="button" role="menuitem" data-column-clear-sort="${key}">Clear Sort</button>` : ''}</div><div class="facility-column-menu-group"><span>Filter</span><button type="button" role="menuitem" data-column-filter="${key}" data-filter-value="all">All</button>${opts.map(o => `<button class="${String(fv) === String(o.value) ? 'selected' : ''}" type="button" role="menuitem" data-column-filter="${key}" data-filter-value="${esc(o.value)}">${esc(o.label)}</button>`).join('')}${fv !== 'all' ? `<button type="button" role="menuitem" data-column-clear-filter="${key}">Clear Filter</button>` : ''}</div>`; }
  function positionAction() { const d = qs('.facility-action-dropdown:not(.hidden)'), t = qs('[data-action-menu-toggle][aria-expanded="true"]'); if (!d || !t) return; d.classList.remove('facility-action-dropdown-up','facility-action-dropdown-down'); d.classList.add((window.innerHeight - t.getBoundingClientRect().bottom) < d.getBoundingClientRect().height + 16 ? 'facility-action-dropdown-up' : 'facility-action-dropdown-down'); }
  ...
  function activeFilters() { return Boolean(state.search.trim()) || ['status','priority','category_id','department_id','assigned_to','sla_status','dateRange'].some(k => state[k] !== 'all'); }
  ...
  function detailsHtml(item) {
    return `<div class="facility-details-modal-panel"><div class="facility-details-modal-header"><div><p>Facility Request</p><span class="facility-details-modal-request-number">${esc(item.request_number)}</span><h2 id="facility-drawer-title">${esc(item.subject || 'Request Details')}</h2></div><button class="facility-details-modal-close" type="button" data-close-drawer aria-label="Close request details">&times;</button></div><div class="facility-details-modal-body"><div class="facility-detail-grid"><section><h3>General Information</h3>${detail('Category', item.category?.name)}${detail('Priority', title(item.priority))}${detail('Status', title(item.status))}${detail('Submitted', fmt(item.created_at))}</section><section><h3>Requester</h3>${detail('Name', item.requested_by?.full_name)}${detail('Employee No.', item.requested_by?.employee_number)}${detail('Department', item.requested_by?.department?.name)}</section><section><h3>Location</h3>${detail('Space', item.location?.space_name || 'Not applicable')}${detail('Building', item.location?.building_name || 'Not applicable')}</section><section><h3>Assignment</h3>${detail('Assigned To', item.assigned_to?.full_name || 'Unassigned')}</section><section><h3>SLA</h3>${detail('Status', title(item.sla?.status || 'NOT_APPLICABLE'))}${detail('Resolution Due', fmt(item.sla?.resolution_due_at))}${detail('Minutes Remaining', item.sla?.minutes_remaining ?? 'Not applicable')}</section><section><h3>Description</h3><p>${esc(item.description || 'No description provided.')}</p></section><section><h3>Approval Summary</h3>${lines((item.approval_summary || []).map(x => `${title(x.approval_status)} - step ${x.current_step_number || 0} of ${x.total_steps || 0}`))}</section><section><h3>Workflow Tasks</h3>${lines((item.workflow_tasks || []).map(x => `${x.task_number}: ${title(x.task_status)} - ${x.task_title}`))}</section><section><h3>AI Recommendations</h3>${lines((item.ai_recommendations || []).map(x => `${title(x.feature_type)} - ${x.generated_summary || x.suggested_priority || 'Recommendation recorded'}`))}</section><section class="facility-detail-wide"><h3>Recent History</h3>${history(item.history || [])}</section></div></div></div>`;
  }
  ...
  function closeDialog() { const d = qs('#facility-dialog'); if (d) { d.hidden = true; d.innerHTML = ''; } }
  ...
  function tableClick(e) {
    ...
    const toggle = e.target.closest('[data-action-menu-toggle]'); if (toggle) { state.openActionMenu = state.openActionMenu === toggle.dataset.actionMenuToggle ? null : toggle.dataset.actionMenuToggle; state.openColumnMenu = null; return renderTable(); }
    const action = e.target.closest('[data-request-action]'); if (!action) return; const id = Number(action.dataset.requestId); state.openActionMenu = null; renderTable();
    const map = { view: () => openDetails(id), history: () => openDetails(id), edit: () => openEdit(id), assign: () => openAssign(id), start: () => transition(id, 'IN_PROGRESS', 'started'), complete: () => transition(id, 'COMPLETED', 'completed'), verify: () => transition(id, 'VERIFIED', 'verified'), close: () => transition(id, 'CLOSED', 'closed'), cancel: () => transition(id, 'CANCELLED', 'cancelled') };
    map[action.dataset.requestAction]?.();
  }
  function docClick(e) {
    ...
    if (e.target.closest('.facility-action-menu') || e.target.closest('.facility-column-menu')) return;
  ...
  async function init() { closeDrawer(); bindEvents(); renderTable(); try { await auth(); if (state.accessDenied) return renderTable(); await loadOptions(); await loadList(); } catch (e) { state.loading = false; renderTable(); toast(errMsg(e)); } }
