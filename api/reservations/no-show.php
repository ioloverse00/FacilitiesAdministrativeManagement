...
try {
    $body = readJsonBody();
    $item = reservationService()->markNoShow((int) reservationIdentifier(), $body['reason'] ?? $body['remarks'] ?? null, $user);
    if ($item === null) jsonResponse(false, 'Reservation not found.', [], 404);
    jsonResponse(true, 'Reservation marked as no-show.', ['item' => $item]);
