...
try {
    $item = employeeReservationService()->checkInOwn((int) employeeReservationIdentifier(), $user);
    if ($item === null) jsonResponse(false, 'Reservation not found.', [], 404);
    jsonResponse(true, 'Reservation checked in.', ['item' => employeeVisibleReservation($item, $user)]);
