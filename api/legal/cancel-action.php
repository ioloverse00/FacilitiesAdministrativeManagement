...
try {
    ...
    $result = legalMatterActionService()->cancelAction($matterId, $actionId, readJsonBody(), $user);
    if ($result === null) {
        jsonResponse(false, 'Legal action not found.', [], 404);
    }
    jsonResponse(true, 'Legal action cancelled.', ['item' => legalMatterService()->show($matterId)]);
} catch (Throwable $e) {
