...
try {
    ...
    $result = legalMatterActionService()->acceptSuggestion($matterId, $suggestionId, readJsonBody(), $user);
    if ($result === null) {
        jsonResponse(false, 'Action suggestion not found.', [], 404);
    }
    jsonResponse(true, 'Action suggestion accepted.', ['item' => legalMatterService()->show($matterId)], 201);
} catch (Throwable $e) {
