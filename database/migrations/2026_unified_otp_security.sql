-- Unified FAM OTP security schema.
-- Additive/idempotent only. Do not store plaintext OTP values.
...
CREATE TABLE IF NOT EXISTS login_mfa_challenge (
    ...
    challenge_uuid CHAR(36) NOT NULL,
    user_account_id BIGINT UNSIGNED NOT NULL,
    pending_session_hash CHAR(64) NOT NULL,
    ...
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (challenge_id),
    UNIQUE KEY uq_login_mfa_challenge_uuid (challenge_uuid),
    KEY idx_login_mfa_active (user_account_id, pending_session_hash, purpose, consumed_at, expires_at),
    CONSTRAINT fk_login_mfa_user FOREIGN KEY (user_account_id) REFERENCES user_account (user_account_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
